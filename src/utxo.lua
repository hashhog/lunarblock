...
function M.new_coin_view(storage)
  local self = setmetatable({}, CoinView)
  self.storage = storage
  self.cache = {}    -- outpoint_key -> utxo_entry or "spent" sentinel
...
function CoinView:get(txid, vout)
  ...
  if entry == "spent" then return nil end
...
function CoinView:flush()
  ...
  batch.destroy()
  self.dirty = {}
  -- Optionally trim cache to limit memory usage
end
