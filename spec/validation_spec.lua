describe("validation", function()
  ...
  -- ===========================================================================
  describe("encode_bip34_height canonical encoder", function()
    it("height 0 → OP_0 (0x00)", function()
      assert.equal("\x00", validation.encode_bip34_height(0))
    end)
    it("height 1 → OP_1 (0x51)", function()
      assert.equal("\x51", validation.encode_bip34_height(1))
    end)
    it("height 16 → OP_16 (0x60)", function()
      assert.equal("\x60", validation.encode_bip34_height(16))
    end)
    it("height 17 → 1-byte push (0x01 0x11)", function()
      assert.equal("\x01\x11", validation.encode_bip34_height(17))
    end)
    it("height 127 → no sign pad (0x01 0x7f)", function()
      assert.equal("\x01\x7f", validation.encode_bip34_height(127))
    end)
    it("height 128 → sign pad at 0x80 (0x02 0x80 0x00)", function()
      assert.equal("\x02\x80\x00", validation.encode_bip34_height(128))
    end)
    it("height 32768 → sign pad at 0x8000 (0x03 0x00 0x80 0x00)", function()
      assert.equal("\x03\x00\x80\x00", validation.encode_bip34_height(32768))
    end)
    it("height 500000 (0x07A120 LE: 0x20 0xA1 0x07)", function()
      assert.equal("\x03\x20\xa1\x07", validation.encode_bip34_height(500000))
    end)

    it("REJECT: length-prefixed [0x01,0x01] != canonical OP_1 for height 1", function()
      assert.not_equal("\x01\x01", validation.encode_bip34_height(1))
    end)
    it("REJECT: zero-padded [0x02,0x64,0x00] != canonical for height 100", function()
      assert.not_equal("\x02\x64\x00", validation.encode_bip34_height(100))
    end)
    it("REJECT: [0x01,0x80] (missing sign byte) != canonical for height 128", function()
