...
describe("main module", function()
  ...
  describe("parse_args", function()
    ...
    it("--externalip: repeatable, comma-separated, turns discover off", function()
      local args = main.parse_args({"--externalip=1.2.3.4,5.6.7.8:9999", "--externalip", "[2600:1700::1]"})
      assert.same({"1.2.3.4", "5.6.7.8:9999", "[2600:1700::1]"}, args.externalip)
      assert.is_false(args.discover)
    end)

    it("--discover defaults on, and stays on when explicit with --externalip", function()
      assert.is_true(main.parse_args({}).discover)
      assert.same({}, main.parse_args({}).externalip)
      assert.is_true(main.parse_args({"--externalip=1.2.3.4", "--discover"}).discover)
      assert.is_false(main.parse_args({"--discover=0"}).discover)
      assert.is_false(main.parse_args({"--nodiscover"}).discover)
    end)
